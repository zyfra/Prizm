...
import { PRIZM_INPUT_DATE_TIME_RANGE_PROVIDERS } from './input-date-time-range.providers';
...
import {
  PRIZM_FIRST_DAY,
  PRIZM_FIRST_DAY_WITH_TIME,
  PRIZM_LAST_DAY,
  PRIZM_LAST_DAY_WITH_TIME,
} from '../../../@core/date-time/days.const';
...
import { PRIZM_RANGE_SEPARATOR_CHAR, PrizmDateTime } from '../../../@core/date-time/date-time';
...
import { PrizmDateTimeRangePeriod } from '../../../@core/classes/date-time-range-period';
    ...
    implements PrizmWithOptionalMinMax<PrizmDateTime>, PrizmFocusableElementAccessor
{
  ...
  @Input()
    @prizmDefaultProp()
    items: readonly PrizmDateTimeRangePeriod[] = [];
  ...
  get activePeriod(): PrizmDateTimeRangePeriod | null {
        return (
            this.items.find(item =>
                prizmNullableSame(
                    ...
                    item.range,
                    (a, b) =>
                        a.dayRange.from.daySame(b.dayRange.from.dayLimit(this.min?.day, this.max?.day)) &&
                        a.dayRange.to.daySame(b.dayRange.to.dayLimit(this.min?.day, this.max?.day)),
                ),
            ) || null
        );
    }
    ...
    public onValueChange(value: string): void {
        ...
        const parsedValue = PrizmDateTimeRange.normalizeParse(value, this.dateFormat);
    ...
    public onRangeChange(range: PrizmDateTimeRange | null): void {
        this.toggle();
        this.focusInput();
        ...
        if (!prizmNullableSame<PrizmDateTimeRange>(this.value, range, (a, b) => a.daySame(b))) {
            this.updateValue(range);
        }
    }

    public onItemSelect(item: string | PrizmDateTimeRangePeriod): void {
        this.toggle();
        this.focusInput();
    ...
    private clampValue(value: PrizmDateTimeRange): PrizmDateTimeRange {
        const clampedBottom =
            this.minLength && value.dayRange.from.append(this.minLength).dayAfter(value.to)
        ...
        return clampedBottom.to.dayAfter(availableMax)
            ? new PrizmDateTimeRange(clampedBottom.from, availableMax)
            : clampedBottom;
    }
