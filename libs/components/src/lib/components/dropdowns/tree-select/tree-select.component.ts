...
export class PrizmInputTreeSelectComponent<T> extends PrizmInputNgControl<T[]> implements AfterViewInit {
  ...
  @Input()
  itemsTemplate = this.options.itemsTemplate;
  ...
  public selectByState(item: T, state: boolean): void {
    const obj = this.filteredItems.find(i => i.obj === item);
    if (!obj) return;
    this.selectItem(obj, true);
  }
  ...
  protected selectItem(item: PrizmMultiSelectItemWithChecked<T>, newItemState = !item.checked): void {
    let values: T[];
    this.markAsTouched();
