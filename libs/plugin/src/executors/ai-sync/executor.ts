...
async function getChangedFiles(
  ...
  try {
    ...
    logger.info(
      `Получение списка измененных файлов между ветками ${colors.yellow}${compareBranch}${colors.reset} и ${colors.yellow}${branch}${colors.reset}`
    );
    const { stdout } = await exec(`git diff --name-status ${compareBranch}...${branch}`, {
      cwd: directoryPath,
    });
    ...
    if (changedFiles.length > 0) {
      ...
      if (logger.logLevel === 'verbose') {
        changedFiles.forEach(file => {
          ...
          logger.verbose(
            `${file.relativePath} (${statusText}, ${file.size ? logger.formatFileSize(file.size) : 'размер неизвестен'})`
          );
        });
      }
    } else {
...
async function uploadFile(file: FileInfo, options: FileSyncExecutorOptions, logger: Logger): Promise<void> {
  return new Promise((resolve, reject) => {
    const keyPath = path.join(options.directory, options.keyPrefix, file.relativePath);
...
export default async function runExecutor(
  ...
  try {
    ...
    if (options.branch && options.compareBranch) {
      logger.info(
        `Режим сравнения веток: ${colors.yellow}${options.compareBranch}...${options.branch}${colors.reset}`
      );
    } else {
    ...
    if (options.branch && options.compareBranch) {
      ...
      filesToSync = filesToSync.filter(
        file =>
      ...
      logger.verbose(
        `Отфильтровано файлов: ${beforeCount} -> ${filesToSync.length} (только из ${options.directory})`
      );
    ...
    if (logger.logLevel !== 'minimal') {
      ...
      Object.entries(extensionStats)
        ...
        .forEach(([ext, stats]) => {
          console.log(
            `   ${colors.yellow}${ext || 'без расширения'}${colors.reset}: ${stats.count} файлов (${logger.formatFileSize(stats.size)})`
          );
        });
      console.log('');
    }
    ...
    logger.info(
      `Начинаем синхронизацию с параллельностью: ${colors.bright}${options.concurrency}${colors.reset}`
    );
